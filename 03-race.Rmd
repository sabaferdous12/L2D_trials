...
Consider the line `Ïµ_values = np.random.randn(100)`.
...
-   The next line creates an empty *list* called `Ïµ_values` that will
...
Consider the statement `Ïµ_values = []`, which creates an empty list.
...
In our program, indentation decreases after line `Ïµ_values.append(e)`,
...
for t in range(T):
    x[t+1] = Î± * np.abs(x[t]) + np.random.randn()
